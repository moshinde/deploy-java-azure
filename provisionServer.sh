...
EOF
echo "Switching user"
su -  azagentuser bash -c 'mkdir azagent;cd azagent;curl -fkSL -o vstsagent.tar.gz https://vstsagentpackage.azureedge.net/agent/2.168.2/vsts-agent-linux-x64-2.168.2.tar.gz;tar -zxvf vstsagent.tar.gz; if [ -x "$(command -v systemctl)" ]; then ./config.sh --deploymentgroup --deploymentgroupname "dev" --acceptteeeula --agent $HOSTNAME --addDeploymentGroupTags --deploymentGroupTags "N" --url https://dev.azure.com/MonicaShinde1307/ --work _work --projectname 'RockNRollTrain' --auth PAT --token ***** --runasservice; sudo ./svc.sh install; sudo ./svc.sh start; else ./config.sh --deploymentgroup --deploymentgroupname "DEV" --acceptteeeula --agent $HOSTNAME --addDeploymentGroupTags --deploymentGroupTags "dev" --url https://dev.azure.com/MonicaShinde1307/ --work _work --projectname 'RockNRollTrain' --auth PAT --token *****; ./run.sh; fi'
